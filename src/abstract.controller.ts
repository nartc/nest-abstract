...
import { AbstractDocument, UpdateResultType, DeleteResultType } from './types';
...
export function abstractControllerFactory<T>(
  ...
  abstract class AbstractController {
    ...
    constructor(service: AbstractCoreService<T>) {
    ...
    public async find(@Query('filter') filter: string): Promise<T[]> {
      ...
      try {
        const resultPromise = this._service.find(findFilter);
    ...
    public async findById(@Param('id') id: string): Promise<T> {
      try {
        const resultPromise = this._service.findById(id);
        return resultPromise;
      } catch (e) {
        throw new InternalServerErrorException(e);
      }
    ...
    public async create(@Body() doc: Partial<T>): Promise<T> {
      try {
        ...
        const resultPromise = this._service.create(
    ...
    public async update(
      ...
      try {
        ...
        const resultPromise = this._service.update(id, updatedDoc);
        return resultPromise;
      } catch (e) {
        throw new InternalServerErrorException(e);
      }
    ...
    public async delete(@Param('id') id: string): Promise<DeleteResultType<T>> {
      try {
        const resultPromise = this._service.delete(id);
        return resultPromise;
      } catch (e) {
        throw new InternalServerErrorException(e);
      }
